...
template <typename T>
typename std::enable_if<bbi::bbi_type<T>::value, std::ostream&>::type operator<<(std::ostream& os,
                                                                                 T const& r) {
    print(r, os);
    return os;
}
...
template <typename T> typename std::vector<T> extract(bbi::record const& r, std::streambuf& s) {
    std::vector<T> rs;
    while (s.in_avail() > 0) {
