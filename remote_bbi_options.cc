...
opts parse_options(int argc, char** argv)
{
  ...
  while ((c = getopt_long_only(argc, argv, "h:p:r:z:m:M:d", longopts, NULL)) != -1)
  {
    switch (c)
    {
    case 'h': opts.host.assign(optarg);
      break;
    case 'p': opts.port.assign(optarg);
      break;
    case 'r': opts.resource.assign(optarg);
      break;
    case 'c': opts.contig.assign(optarg);
      break;
    case 'z': opts.zoom_level = std::atoi(optarg);
      break;
    case 'm': opts.m = std::atoi(optarg);
      break;
    case 'M': opts.M = std::atoi(optarg);
      break;
    case 'd':
    ...
    case 0:
      switch (print)
      {
      case 1: opts.print_chrom_ids = true;
        break;
      case 2: opts.print_main_headers = true;
        break;
      case 3: opts.print_zoom_headers = true;
        break;
      default:
        break;
      }
