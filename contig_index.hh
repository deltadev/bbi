...
public:
  ...
  friend
  void print_header(contig_index const& index, std::ostream& os)
  { print(index.header, os); }
  
  friend
  int contig_id(contig_index const& index, std::string key)
  { return index.id_hash.at(key).chrom_id; }
  
  friend
  int contig_size(contig_index const& index, std::string key)
  { return index.id_hash.at(key).chrom_size; }
  
  friend
  iterator begin(contig_index const& index) { return std::begin(index.id_hash); }
  
  friend
  iterator end(contig_index const& index) { return std::end(index.id_hash); }
  
  friend
  std::ostream& operator<<(std::ostream& os, contig_index const& index)
  {
